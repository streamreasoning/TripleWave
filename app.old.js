...
var primus = Primus.createServer(function connection(spark){
  ...
  root: configuration.get('ws_stream_location'),
  transformer: 'websockets'
});

primus.on('connection', function (spark) {
  scheduler.pipe(spark);
});

primus.on('disconnection', function (spark) {
  scheduler.unpipe(spark);
});
...
var createNewGraphs = function(callback) {
    ...
    client
        ...
        .execute(function(err, data) {
            ...
            console.log(graphs);
            async.eachSeries(graphs, createNewTriples, function(err) {
                return callback(err);
            });
        });
...
app.listen(app.get('port'), function() {
    ...
    if (configuration.get('mode') === 'transform') {
        ...
        if (configuration.get('sources') == 'rdfstream') {
            ...
            console.log('Fiole path', filePath);
            var file = fs.createReadStream(filePath)
            console.log("sono bello")
            oboe(file)
                .node('![*]', function(graph) {
